...
Liste_Contours ajouter_element_liste_Contours(Liste_Contours L, Liste_Point contour) {
    Cellule_Liste_Contours *nouveau = (Cellule_Liste_Contours *)malloc(sizeof(Cellule_Liste_Contours));
    if (!nouveau) {
        exit(EXIT_FAILURE);
    }
    nouveau->contour = contour;
    nouveau->suiv = NULL;
    ...
    L.last = nouveau;
    L.taille++;
    return L; // Important : retourner la liste mise à jour
}


Liste_Contours concatener_liste_Contours(Liste_Contours L1, Liste_Contours L2) {

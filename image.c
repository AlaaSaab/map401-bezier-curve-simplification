...
{
	printf("P1\n");
    printf("# pixels séparés par des espaces");
    printf(" %u 	%u \n", I.la_largeur_de_l_image, I.la_hauteur_de_l_image);

    // Parcourir les pixels et les afficher
    for (UINT h = 1; h <= I.la_hauteur_de_l_image; h++) {
        for (UINT l = 1; l <= I.la_largeur_de_l_image; l++) {
            // Afficher chaque pixel (0 ou 1)
            printf("%d ", get_pixel_image(I, l, h));
        }
...
{
    Image negatif = creer_image(I.la_largeur_de_l_image, I.la_hauteur_de_l_image);

    for (int h = 1; h <= I.la_hauteur_de_l_image; h++) {
        for (int l = 1; l <= I.la_largeur_de_l_image; l++) {
            Pixel original_pixel = get_pixel_image(I, l, h);
			Pixel negatif_pixel;
            if (original_pixel == BLANC) {
                negatif_pixel = NOIR;
            } else {
                negatif_pixel = BLANC;
            }
            set_pixel_image(negatif, l, h, negatif_pixel);
        }
    }
